...
class RelabelToFrontTest extends Specification {
    def "Google Code Jam 2016 Round 1B Problem C. Technobabble を解かせる"() {
        ...
        testCaseNum.times {
            ...
            RelabelToFront dinitz = new RelabelToFront(V)
            for (int i = 0; i < N; i++) {
    ...
    def "Solve AOJ GRL_6_A"() {
        ...
        while (counter.hasNext()) {
            ...
            RelabelToFront relabelToFront = new RelabelToFront(V)
            E.times {
                int u = Integer.parseInt(inQ.poll())
                int v = Integer.parseInt(inQ.poll())
                int c = Integer.parseInt(inQ.poll())
                relabelToFront.addEdge(u, v, c)
            }
            long ans = relabelToFront.maxFlow(0, V - 1)
